...
public class SEOUL14_ANTAEHYUN {

	// 닉네임을 사용자에 맞게 변경해 주세요.
	static final String NICKNAME = "SEOUL14_ANTAEHYUN";
	...
	public static void main(String[] args) {
		...
		try {
			...
			while (socket != null) {
				...
				/*
				 * 타겟 공이 2사분면이 일때를 추가하여 2사분면인 목적구도 흰 공이 칠 수 있게 수정.
				 * 공번호가 1-3-5순으로 들어가게끔 코드 추가
				 */
				...
				if(order == 1) {
					...
					if(targetBall_x < 0) {	//1번째 공이 들어갔다면 (-1 되어버리니까)
						//3번 공 정보 알아오기
						targetBall_x = balls[3][0];
						targetBall_y = balls[3][1];
					}
					
					if(targetBall_x < 0) {	//3번 공도 들어갔다면 (-1 되어버리니까)
						//5번 공 정보 알아오기
