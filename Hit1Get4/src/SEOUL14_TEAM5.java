...
public class SEOUL14_TEAM5 {

	// 닉네임을 사용자에 맞게 변경해 주세요.
	static final String NICKNAME = "SEOUL14_PARKHOHYUN";
	...
	public static void main(String[] args) {
		...
		try {
			...
			while (socket != null) {
				...
				for (int i = 1; i < balls.length; i++) { // 모든 공을 탐색하며,
					if ((order == 1 && (i == 1 || i == 3)) || (order == 2 && (i == 2 || i == 4))) {
						if (balls[i][0] != -1.0 && balls[i][1] != -1.0) { // 좌표가 (-1, -1)인 경우 스킵
				...
				// 목적구가 흰 공을 중심으로 2사분면에 위치했을 때 각	도를 재계산
				if (whiteBall_x > targetBall_x && whiteBall_y < targetBall_y)
				{
					radian = Math.atan(height / width);
					angle = (float) (((180.0 / Math.PI) * radian) + 270 - 0.15); // 0.15도 차이가 날 수 있도록 구현
				}
...
//				power = Math.min((Math.max((float) distance - 15, 10)), 100); // 힘의 세기 조절
